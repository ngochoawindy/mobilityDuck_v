...
// ----------------------------------------------------------------------
// SRID CSV configuration for MEOS (with optional embedding)
// ----------------------------------------------------------------------
...
#if defined(MDUCK_EMBED_SRID_CSV)

// These are generated by CMake + xxd and wrapped in `namespace duckdb`
extern unsigned char spatial_ref_sys_csv[];
extern unsigned int spatial_ref_sys_csv_len;
...
static void ConfigureMeosSridCsvOnce() {
    ...
    std::call_once(once, [] {
...
#if defined(MDUCK_EMBED_SRID_CSV)
        // 1. If user explicitly set env var and file exists, honour that
        if (env_path && *env_path && file_exists(env_path)) {
            chosen = env_path;
        } else {
            // 2. Otherwise, materialise the embedded CSV to a temp file
            static std::string embedded_path = EnsureEmbeddedSridCsvOnDisk();
            chosen                           = embedded_path.c_str();
        }
#else
        // No embedding: use env var or the compiled default path
        if (env_path && *env_path && file_exists(env_path)) {
            chosen = env_path;
        ...
        if (chosen) {
            ...
            fprintf(
                stderr,
                "[mobilityduck] WARNING: no spatial_ref_sys.csv found "
                "(env %s or default path \"%s\")\n",
                MDUCK_SRID_ENV_NAME,
                MDUCK_DEFAULT_SRID_CSV
            );
        }
    });
}
...
static void LoadInternal(DatabaseInstance &instance) {
	// Configure MEOS SRID CSV once (env / embedded / default path)
